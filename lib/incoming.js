...
const data        = [
  {
    question: "Help",
    answers: [{
      text: function () { return data.reduce( (prev, curr) => `${prev} \n ${curr}`) }
    }]
  },
  {
...
let sendMessage = function (sender, answers, i) {
  ...
  if(typeof messageData.text === 'function') messageData = messageData.text();
  request({
...
let reply = function(sender, text) {
  let result = data.find(el => el.question === text);
  if(result) {
